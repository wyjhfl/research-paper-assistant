...
- **E2E / 全量 pytest 在 RC gate 中执行**：日常开发使用 quick gate 或窄验证；Phase 43 执行了 RC gate 的可运行部分，Phase 44 补跑 Playwright E2E 通过
...
### Phase 43–44 RC Gate 结果

Phase 43 执行了 RC gate 的可运行部分（6/7 步通过）；Phase 44 在宿主机配置 Node.js 后补跑 Playwright E2E，103/103 全部通过。E2E 例外已消除。
...
| Frontend build | ✅ npm run build 通过 (Phase 44) |
| Playwright E2E | ✅ 103 passed / 0 failed (Phase 44 补跑) |
...
> Phase 43–44 RC gate 全部 7 步通过（含 Phase 44 补跑 Playwright E2E）。详细证据见 RC_EVIDENCE_v1.0.0-rc.1.md。
