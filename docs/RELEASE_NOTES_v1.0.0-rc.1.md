...
v1.0.0-rc.1 是多 Agent 科研论文助手的第一个 Release Candidate。本版本包含完整的论文处理、RAG 问答、多 Agent 工作流、真实认证、后台任务、运维工具链和生产化门禁。
...
- **E2E / 全量 pytest 在 RC gate 中执行**：日常开发使用 quick gate 或窄验证；Phase 43 执行了 RC gate 的可运行部分，Playwright E2E 因环境限制未执行（见已知限制）
...
- **Phase 43 Playwright E2E 未执行（RC 发布例外）**：主机无 npm/node，前端容器为生产 runner 不含 dev 工具；7 个 E2E spec 文件存在且配置正确，待有 npm 环境时补跑。此为当前 RC 的已知例外，非代码缺陷。
...
### Phase 43 RC Gate 结果（E2E 例外）

Phase 43 执行了 RC gate 的可运行部分；Playwright E2E 因环境限制未在 Phase 43 执行。

| 步骤 | 结果 |
|------|------|
| Documentation secret scan | ✅ PASS |
| Frontend mojibake scan | ✅ PASS |
| Production check | ✅ 19/19 ALL CHECKS PASSED (Phase 43) |
| Alembic current | ✅ 003_job_runs (head) |
| Backend pytest | ✅ 453 passed / 22 skipped / 0 failed |
| Frontend build | ✅ Docker 多阶段构建已验证 |
| Playwright E2E | ⚠️ 未执行（环境限制，见已知限制） |
| Backup manifest validate | ✅ ok: true, errors: [], warnings: [] |
| Restore dry-run | ✅ DB + Storage + Eval 均找到，无数据修改 |

### 历史验收记录

- **Phase 41 quick gate**：secret scan ✅ / mojibake scan ✅ / production check PASS ✅ / Alembic at head ✅
- **Phase 41.2 窄验证**：env_example / quick_gate / checklist levels / runbook recovery 相关检查通过
- **Phase 41 RC gate**：production check PASS / 420 pytest / 103 E2E（Phase 41 历史结果，非 Phase 43 新验收）
- **Docker 全量 pytest**：Phase 41.1 因 test DB 残留连接卡死，已恢复并记录到 OPERATIONS_RUNBOOK

> Phase 43 执行了 RC gate 的可运行部分（6/7 步通过，Playwright E2E 因环境限制未执行）。详细证据见 RC_EVIDENCE_v1.0.0-rc.1.md。
