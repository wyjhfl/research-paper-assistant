...
- **当前能力**：
  ...
  - 单论文 RAG 问答（向量检索 + hybrid lexical retrieval + evidence gate + LLM 生成）
  - 跨论文 RAG 问答（多论文联合检索 + hybrid lexical retrieval + per_paper_limit + paper_title 来源）
  - 中文问题跨语言关键词扩展（静态中英学术词典，仅用于 lexical scoring，不改变原始问题）
...
# 个人本地工作流烟测（默认不写入研究笔记；显式 --write-note 才保存本地 smoke note）
python scripts/personal_workflow_smoke.py
python scripts/personal_workflow_smoke.py --write-note

# 一键本地检查 + 可选个人工作流闭环烟测
# 覆盖 Git/Docker/health/ready/前端核心路由关键文案/papers/ideas/notes/jobs/smoke/security
